"""
Agent/Tool-Use Training Module for DeepSeek-V3.2 (MLX)

Apple Silicon optimized implementation of agent training with:
...
class AgentRewardComputer:
    """Compute rewards for agent trajectories (MLX optimized)"""
...
class AgentEnvironment:
    """Simulated environment for agent training (MLX optimized)"""
...
# ============================================================================
# Agent GRPO Trainer (Extended for Multi-Turn) - MLX Optimized
# ============================================================================
...
class AgentGRPOTrainer:
    """Extended GRPO trainer for multi-turn agent trajectories (MLX)"""
    ...
    def compute_loss(
        ...
        """Compute GRPO loss with multi-turn credit assignment (MLX optimized)
        ...
        # 1. Compute log probabilities using MLX
        log_probs = mx.log(mx.softmax(logits, axis=-1) + 1e-10)
        
        # Gather token log probs (MLX doesn't have gather, use take_along_axis equivalent)
        input_ids_expanded = mx.expand_dims(input_ids, axis=-1)
        token_log_probs = mx.take_along_axis(log_probs, input_ids_expanded, axis=-1)
        token_log_probs = mx.squeeze(token_log_probs, axis=-1)  # (G, Seq)
        ...
        if turn_boundaries is not None:
            ...
            seq_log_probs = mx.sum(weighted_log_probs, axis=1)  # (G,)
        else:
    ...
    def _compute_turn_advantages(
        ...
        for i, boundaries in enumerate(turn_boundaries):
            ...
            if num_turns == 0:
                turn_advantages.append(mx.array([1.0]))
                continue
            ...
            if self.config.turn_credit_method == "exponential":
                ...
                credits = mx.ones(num_turns)
            else:  # final_only
                credits = mx.zeros(num_turns)
                credits = credits.at[-1].add(1.0)
            
            # Normalize
            credits = credits / (mx.sum(credits) + 1e-8)
    ...
    def _apply_turn_weights(
        self,
        token_log_probs: mx.array,
        turn_advantages: List[mx.array],
        turn_boundaries: List[List[Tuple[int, int]]]
    ) -> mx.array:
        ...
        weighted_np = [[0.0] * Seq for _ in range(G)]
        token_np = token_log_probs.tolist()
        ...
        for i, (boundaries, advantages) in enumerate(zip(turn_boundaries, turn_advantages)):
            ...
            for j, (start, end) in enumerate(boundaries):
                if j < len(adv_list):
                    for k in range(start, min(end, Seq)):
                        weighted_np[i][k] = token_np[i][k] * adv_list[j]
        
        return mx.array(weighted_np)
    ...
    def get_training_progress(self) -> Dict[str, Any]:
        ...
        tier_total = threshold - prev_threshold if threshold != float('inf') else 10000
...
# ============================================================================
# Group Sampler for Agent Tasks (MLX)
# ============================================================================
